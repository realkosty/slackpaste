...
function CopyButton({ pasted, disabled }) {
  ...
  return (
    <p>
      <button className="copy-button" onClick={handleCopy} disabled={disabled}>
...
function App() {
  ...
  React.useEffect(() => {
    if (originalDoc) {
      let [processed, attachmentsDetected] = processDoc(originalDoc, {
        anonymize,
        includeChannelId,
        csvExportFriendly,
      });
      ...
      if (attachmentsDetected > 0) {
        ...
        alert(
          `${attachmentsDetected} ${attachmentGrammar} detected. Remember to download image files and manually include them in notion.`,
        );
      }
    } else {
      setDisabled(true);
