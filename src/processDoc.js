...
const processDoc = (inputDoc, { anonymize, includeChannelId, csvExportFriendly }) => {
  ...
  let blocksWithPossibleBrs = [
    ...doc.querySelectorAll(MESSAGE_SECTION_SELECTOR),
    ...doc.querySelectorAll(MESSAGE_QUOTE_SELECTOR),
  ];
  ...
  if (anonymize) {
    ...
    function hasExtIcon(e) {
      // external members of Slack Connect channels will have an icon
      let parentPrevSibl = e.parentElement.previousElementSibling;
      if (parentPrevSibl) {
        // "Compact" message display style (Preferneces -> Messages & media -> Theme)
        return parentPrevSibl.matches(SLACK_CONNECT_EXT_ICON_SELECTOR);
      } else {
        // "Clean" message display style
    ...
    names.forEach(e => {
      ...
      if (!placeholders.hasOwnProperty(name)) {
        ...
        placeholders[name] = p;
        i++;
      } else if (placeholders[name] === 'OP') {
        // sometimes the copied block does not include first sender's icon even if it's there
        if (e.matches(SENDER_SELECTOR)) {
          if (hasExtIcon(e)) {
  ...
  for (const elm of doc.querySelectorAll(MESSAGE_CLASS)) {
    ...
    if (timestampLink) {
      ...
      // e.g. Mar 23, 2023
      const humanReadableTimestamp = new Date(timestamp * 1000).toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
      });
  ...
  debugDoc.querySelectorAll('*').forEach(e => {
    // remove attributes for readability
